...
board_drv_t &board_drv_t::get_instance(const role_t role, const can_hub_t::which_can can_ch)
{
...
void board_drv_t::init_impl() const
{
    ...
    else
    {
        ...
        const auto shoot_cnt = static_cast<uint8_t>((sizeof(event_shoot_t) + 7) / 8);
    ...
    while (true)
    {
        ...
        if (_role == role_t::CHASSIS)
        {
            ...
            for (uint8_t i = 0; i < G2C_FRAME_CNT; ++i)
            {
                ...
                {
                    auto copy_len = static_cast<uint8_t>(sizeof(g2c_data_t) - (i * 8));
        ...
        else
        {
            ...
            for (uint8_t i = 0; i < C2G_FRAME_CNT; ++i)
            {
                ...
                {
                    auto copy_len = static_cast<uint8_t>(sizeof(c2g_data_t) - (i * 8));
                    if (copy_len > 8) copy_len = 8;
                    memcpy(ptr + (i * 8), raw_data.data(), copy_len);
                    _is_online       = true;
                    _last_rx_time_ms = current_time;
...
status_t board_drv_t::send_event_raw(const uint32_t event_base_id, const void *data, const size_t size) const
{
    auto *can_obj = can_hub_t::get_instance()->hub_get_can_obj(_can_ch);
    if (!can_obj || !data) return PYRO_ERROR;
...
bool board_drv_t::read_event_raw(const uint32_t event_base_id, void *data_out, const size_t size) const
{
