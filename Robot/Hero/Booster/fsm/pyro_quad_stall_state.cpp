...
void quad_booster_t::fsm_active_t::state_stall_t::enter(owner *owner)
{
    ...
    if (&owner->_state_active._homing_state == owner->_state_active._last_state)
    {
        ...
        // 2. 拨叉有6个槽位，间距为 PI/3。严格寻找正转方向上的最近预置位
        ...
        const float forward_delta =
            (TRIGGER_FEED_DIR < 0.0f)
                ? delta - n * trigger_slot_rad
                : n * trigger_slot_rad - delta;
        if (forward_delta < TRIGGER_PRESET_MIN_ADVANCE_RAD)
        {
            n += (TRIGGER_FEED_DIR < 0.0f) ? -1.0f : 1.0f;
        }

        // 3. 计算出最近的绝对目标角度
        owner->_ctx.data.target_trig_rad = TRIGGER_OFFSET + n * trigger_slot_rad;
